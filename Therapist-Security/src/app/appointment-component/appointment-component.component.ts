...
export class AppointmentComponentComponent {
  ...
  updateAppointment(appointment:Appointment){
    ...
    console.log(body)
    return this.http.post(constants.baseURL + 'appointment' + appointment.id +'/', body,{'headers':headers})
  }
  ...
  doGetAppointment(id:number):Observable<Appointment> {
    ...
    console.log(body);
    return this.http.get<Appointment>(constants.baseURL + 'appointment/'+id +'/')
  }
  ...
  isCheckoutButtonDisabled():boolean{
    if (this.appointment.status ==  constants.appointStatusInProgress || 
